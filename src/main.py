...
def generer_rapport():
    ...
    for ticker, (prix_achat, qte) in PORTEFEUILLE.items():
        try:
            ...
            analyse = ""
            if abs(var_jour) > 5:
                analyse = analyser_actus(
                    ticker, var_jour
                )  # On passe la variation du jour à l'IA
    ...
    pct_profit_global = (
        100
        * total_profit_global
        / sum(prix * quantite for prix, quantite in PORTEFEUILLE.values())
    )
    corps_mail += "------------------------------\n"
...
def envoyer_mail(contenu, image_paths=None):
    ...
    password = os.getenv("EMAIL_PASSWORD") or (
        config.EMAIL_PASSWORD if config else None
    )
    receiver = os.getenv("EMAIL_RECEIVER") or (
        config.EMAIL_RECEIVER if config else None
    )
